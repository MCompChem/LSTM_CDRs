...
flags.add_argument("-t", "--train", default=True, help="whether the network should be trained or just sampled from", type=str)
...
flags.add_argument("-f", "--fminlen", default=7, help="minimum sequence length allowed when sampling new sequences", type=int)
...
flags.add_argument("--finetune", default=False, help="if True, a pretrained model provided in modfile is finetuned on the dataset", type=str)
...
flags.add_argument("--refs", default=True, help="whether reference sequence sets should be generated for the analysis", type=str)
...
def main(infile, sessname, neurons=64, layers=2, epochs=100, batchsize=128, window=0, step=1, target='all',
         ...
         modfile=None, samplelength=36, minlen=7, pad=0, l2_rate=None, cv=None, finetune=False, references=True):
    ...
    data.generated = model.sample(sample, start=aa, maxlen=samplelength, minlen=minlen, show=False, temp=temperature)
...
if __name__ == "__main__":
    ...
    main(infile=args.dataset, sessname=args.name, batchsize=args.batch_size, epochs=args.epochs,
         ...
         learningrate=args.lr, cv=args.cv, samplelength=args.maxlen, minlen=args.fminlen, window=args.window,
